...
const Courses: FC = () => {
  ...
  const fetchTasks = async () => {
    ...
    const trackingRes = await fetch(
      `${allJson}/student_assignments?student_id=${currentUser.id}`
    );
    ...
    const combined: StudentTask[] = trackingData
      .map((track) => {
        const details = majorAssignments.find(
          (a) => Number(a.task_number) === Number(track.task_number)
        );

        const prEntry = prData.find(
          (pr: any) => Number(pr.task_number) === Number(track.task_number)
        );
